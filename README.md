...
### AI Config Is Shared Between Tools
All AI-agent configuration now lives in the shared `ai/` folder — both OpenCode and Gemini CLI symlink into it. `opencode/` is a symlink to `ai/` (so `~/.config/opencode/` → `opencode/` → `ai/`). Only `AGENTS.md` and `GEMINI.md` at the repo root remain tool-specific (auto-discovered by each CLI).

### AI Runtime Files Are Ignored by Git
`ai/node_modules/`, `ai/package.json`, `ai/opencode.jsonc`, and `ai/skills/graphify/` are in `.gitignore`. These are installed by the tools themselves and are machine-specific.
...
| `ai/` | AI agent config (shared) | Config shared between OpenCode, Gemini CLI, and other AI coding agents |
| `opencode/ → ai/` | OpenCode AI (symlink) | Symlink to `ai/` so `~/.config/opencode/` stays symlinked to the repo |
